...
impl<F: FieldExt, const NETWORK: NetworkArch, const MAX_MAT_WIDTH: usize> Circuit<F>
    for LenetCircuit<F, NETWORK, MAX_MAT_WIDTH>
{
    ...
    fn without_witnesses(&self) -> Self {
        ...
        LenetCircuit {
            layers,
            input: vec![Value::unknown(); NETWORK[0].0],
        }
    }
            ...
            input = if *relu {
                ...
                let inter = input.unwrap_or_else(|| {
                    chip.load_input_instance(
                        layouter.namespace(|| "Load input from instance"),
                        config.input,
                        0,
                        self.input.len(),
                    )
...
fn bench_layer<const NETWORK: NetworkArch, const MAX_MAT_WIDTH: usize>(
    file_path: &str,
    k: u32,
) -> Result<(), PlonkError> {
    ...
    let circuit = LenetCircuit::<Fr, NETWORK, MAX_MAT_WIDTH> {
        layers,
        input: input.iter().map(|x| Value::known(*x)).collect(),
    };
