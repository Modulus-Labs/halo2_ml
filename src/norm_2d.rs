...
pub struct Normalize2dChip<F: FieldExt, const BASE: usize, const K: usize> {
    config: Normalize2dConfig<F>,
}

impl<F: FieldExt, const BASE: usize, const K: usize> Chip<F> for Normalize2dChip<F, BASE, K> {
...
impl<F: FieldExt, const BASE: usize, const K: usize> Normalize2dChip<F, BASE, K> {
...
mod tests {
    ...
    impl<F: FieldExt> Circuit<F> for Norm2DTestCircuit<F> {
        ...
        fn synthesize(
            ...
            let norm_chip: Normalize2dChip<F, 1024, 2> =
    ...
    fn test_simple_norm() -> Result<(), PlonkError> {
        ...
        let mut input_instance = vec![vec![Fr::from(1_048_576); INPUT_HEIGHT]; INPUT_WIDTH];
        let mut output_instance = vec![vec![Fr::one(); INPUT_HEIGHT]; INPUT_WIDTH];
